...
export const generateReview = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { text } = await generateText({
      ...
      prompt: `Patient intake transcript:\n\n${data.transcript}\n\n${langInstruction}\n\nReturn JSON exactly matching:\n{\n  "chiefComplaint": string,\n  "symptoms": string[],\n  "duration": string,\n  "severity": string,\n  "riskLevel": "Low" | "Medium" | "High" | "Critical",\n  "riskScore": integer 0-100,\n  "clinicalNotes": string,\n  "aiSummary": string,\n  "primarySpecialty": string,\n  "secondarySpecialty": string,\n  "alternativeSpecialty": string,\n  "reasoning": string,\n  "isEmergency": boolean,\n  "emergencyReasons": string[]\n}\n\nRISK SCORING — "riskScore" is an integer 0-100 representing overall clinical urgency. Guidance: 0-24 = Low (minor/self-limiting), 25-49 = Medium (warrants timely outpatient care), 50-74 = High (needs prompt specialist attention within 24-48h), 75-100 = Critical (emergency / immediate care). "riskLevel" MUST match the band of "riskScore".\n\nEMERGENCY SCREENING — Set "isEmergency" to true if the transcript suggests ANY of: severe chest pain or pressure; difficulty breathing / shortness of breath at rest; stroke-like symptoms (facial droop, one-sided weakness, slurred speech, sudden severe headache, sudden vision loss); loss of consciousness, fainting, or unresponsiveness; severe / uncontrolled bleeding; suicidal thoughts, self-harm, or intent to harm others; severe allergic reaction / anaphylaxis; signs of sepsis or severe dehydration in infants. When true, list short human-readable reasons in "emergencyReasons" (e.g. "Severe chest pain", "Suicidal ideation"). When false, return an empty array. Whenever "isEmergency" is true, set "riskLevel" to "Critical" and "riskScore" >= 85.\n\nSpecialty names should be one of common specialties: Internal Medicine, Pediatrics, Dermatology, Cardiology, Psychiatry, ENT, Orthopedics, Family Medicine, Neurology, Gastroenterology, Endocrinology, Pulmonology, Urology, Ophthalmology, Gynecology, Emergency Medicine. For emergency cases use "Emergency Medicine" as the primarySpecialty. Note: riskLevel and specialty names MUST stay in English. emergencyReasons SHOULD be in the requested locale.`,
    });
